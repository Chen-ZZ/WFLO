...
public class MyDenseInstance extends DenseInstance {
	...
	/**
	 * Constructor that copies the attribute values and the weight from the given instance. It does NOT
	 * perform a deep copy of the attribute values if the instance provided is also of type MyDenseInstance
	 * (it simply copies the reference to the array of values), otherwise it does. Reference to the dataset is
	 * set to null. (ie. the instance doesn't have access to information about the attribute types)
	 * 
	 * @param instance
	 *            the instance from which the attribute values and the weight are to be copied
	 */
	// @ ensures m_Dataset == null;
	public MyDenseInstance(/* @non_null@ */Instance instance) {
		super(instance);
		s_numericAfterDecimalPoint = 12;
	}

	/**
	 * Constructor that inititalizes instance variable with given values. Reference to the dataset is set to
	 ...
	 */
	// @ ensures m_Dataset == null;
	public MyDenseInstance(double weight, /* @non_null@ */double[] attValues) {
		super(weight, attValues);
		s_numericAfterDecimalPoint = 12;
	}

	/**
	 * Constructor of an instance that sets weight to one, all values to be missing, and the reference to the
	 * dataset to null. (ie. the instance doesn't have access to information about the attribute types)
	 * 
	 * @param numAttributes
	...
	/**
	 * Produces a shallow copy of this instance. The copy has access to the same dataset. (if you want to make
	 * a copy that doesn't have access to the dataset, use <code>new MyDenseInstance(instance)</code>
	 * 
	 * @return the shallow copy
	 */
	// @ also ensures \result != null;
	// @ also ensures \result instanceof MyDenseInstance;
	// @ also ensures ((MyDenseInstance)\result).m_Dataset == m_Dataset;
	@Override
	public/* @pure@ */Object copy() {

		MyDenseInstance result = new MyDenseInstance(this);
	...
	/**
	 * Merges this instance with the given instance and returns the result. Dataset is set to null. The
	 * returned instance is of the same type as this instance.
	 * 
	 * @param inst
	...
	public Instance mergeInstance(Instance inst) {
		...
		return new MyDenseInstance(1.0, newVals);
	}
	...
	/**
	 * Replaces all missing values in the instance with the values contained in the given array. A deep copy
	 * of the vector of attribute values is performed before the values are replaced.
	 * 
	 * @param array
	...
	/**
	 * Sets a specific value in the instance to the given value (internal floating-point format). Performs a
	 * deep copy of the vector of attribute values before the value is set.
	 * 
	 * @param attIndex
	 *            the attribute's index
	 * @param value
	 *            the new attribute value (If the corresponding attribute is nominal (or a string) then this
	 *            is the new value's index as a double).
	 */
	@Override
	...
	/**
	 * Sets a specific value in the instance to the given value (internal floating-point format). Performs a
	 * deep copy of the vector of attribute values before the value is set. Does exactly the same thing as
	 * setValue().
	 * 
	 * @param indexOfIndex
	...
	/**
	 * Returns the description of one instance (without weight appended). If the instance doesn't have access
	 * to a dataset, it returns the internal floating-point values. Quotes string values that contain
	 * whitespace characters.
	 * 
	 * This method is used by getRandomNumberGenerator() in Instances.java in order to maintain backwards
	 ...
	 * @param attIndex
	 *            the attribute's index
	 * @return the specified value as a double (If the corresponding attribute is nominal (or a string) then
	 *         it returns the value's index as a double).
	 */
	@Override
	...
	public static void main(String[] options) {
		...
		try {
			...
			Instance inst = new MyDenseInstance(3);
